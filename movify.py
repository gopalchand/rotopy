...
try:
        ...
        if png_file.endswith(".png"):
            ...
            if os.path.exists(json_file):
                ...
                if annotate_mode is True:
                    ...
                    image = cv2.putText(image, text_to_draw, (text_offset_x,text_offset_y), fontScale = 0.8, fontFace = cv2.FONT_HERSHEY_PLAIN, \
    ...
    if framerate_val != None:
        ...
        ffmpeg_cmd = f'type *jpg | d:\\ffmpeg\\bin\\ffmpeg.exe -loglevel {loglevel}-framerate {framerate_val} -vcodec mjpeg -an -f image2pipe -i - -pix_fmt yuvj420p {output_file}'
    else:
        ffmpeg_cmd = f'type *jpg | d:\\ffmpeg\\bin\\ffmpeg.exe -loglevel {loglevel} -vcodec mjpeg -an -f image2pipe -i - -pix_fmt yuvj420p {output_file}'
    print(f"about to run {ffmpeg_cmd}")
    try:
        completed_process = subprocess.run(ffmpeg_cmd, shell=True, check=True)        
